...
spreadsheet **tables**, presentation **decks**, and **images** — and a deterministic **query
script** is your hands for pulling exact, cited content. Never answer corpus questions from
memory — route, query, then compose.
...
1. **Read the index.** Load `produced_data/cards/index.json`. Each entry has `table_id`,
   `kind` (`table` | `presentation` | `image`), `summary`, `use_cases`, `entity_key_col`,
   `as_of`, `as_of_basis` (stated/inferred/unknown), `joins`, `duplicate_of`; deck/image
   entries also carry `entities`, and decks carry `n_slides`.
...
6. **Emit a QUERY SPEC** (JSON) against real columns and run it:
   ...
   **For a deck** (`kind: presentation`), emit a DECK SPEC with `deck_id` instead — it
   ...
   card's `slides[].point` to pick which slide(s) to pull, and `entities[]` for who/why. Each
   returned slide carries its own `as_of`/`as_of_basis`.

   **For an image** (`kind: image`), emit an IMAGE SPEC with `image_id` — it returns the
   grounded transcript (OCR + visual read captured at ingest), summary, entities, and
   `content_dates`:
   ```
   echo '{"image_id":"I1.image","fields":["summary","transcript","content_dates"]}' \
        | python3 code/tam/query.py
   ```
...
- **Date every answer, and say how you know the date.** The query result carries `as_of`,
  `as_of_basis`, and `staleness`. Lead with it: *"As of **Aug 2020** (F1)…"*. When the date is
  `inferred` (not `stated`), say so — *"(vintage inferred)"* — and when it's `unknown`, flag
  that the date is unconfirmed. For decks/images the date can be **per datapoint**: use the
  slide's own `as_of` or the image's `content_dates` (each with its `basis`) rather than only
  the document vintage. When you combine sources of different vintages, state the range and
  name the oldest. When `staleness.stale` is true, append the `staleness.note`.
- **Cite every fact.** For a table, trace to `file_alias / sheet / row` using the result's
  ...
  `file_alias / slide N` using `provenance_slides` (e.g. `P1/slide 4`). For an image, cite
  `file_alias / image` using `provenance` (e.g. `I1/chart.png`). No citation → don't state it.

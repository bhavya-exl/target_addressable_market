...
2. **What data is here.** Read `produced_data/cards/index.json`; give a one-liner
   (N tables across N documents; newest & oldest vintage). Offer the full breakdown via the
   `tam-catalog` skill.
3. **Offer next steps:** ask a question (`tam-ask`), build a report (`tam-report`), add data
   (`tam-ingest`), or add/update a skill.
